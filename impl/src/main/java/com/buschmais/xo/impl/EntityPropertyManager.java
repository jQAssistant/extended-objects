...
public class EntityPropertyManager<Entity, Relation, PropertyMetadata> extends AbstractPropertyManager<Entity, Relation, Entity> {
    ...
    private Relation createSingleReference(Entity sourceEntity, AbstractRelationPropertyMethodMetadata<?> metadata, Entity targetEntity,
        ...
        if (existingRelation != null) {
            Entity referencedEntity = getReferencedEntity(existingRelation, metadata.getDirection());
            entityInstanceManager.updateInstance(referencedEntity);
            removeRelation(sourceEntity, existingRelation, metadata);
        }
