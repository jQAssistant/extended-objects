...
public class QualifiedRelationTest extends AbstractNeo4jXOManagerTest {
	...
	public void manyToMany() {
		...
		assertThat(executeQuery("MATCH (a:A)-[:ManyToMany]->(b:B) RETURN a, collect(b) as listOfB ORDER BY ID(a)").<A> getColumn("a"),
				hasItems(a1, a2));
		assertThat(executeQuery("MATCH (a:A)-[:ManyToMany]->(b:B) RETURN a, collect(b) as listOfB ORDER BY ID(a)")
				.<Iterable<B>> getColumn("listOfB"), hasItems(hasItems(b1, b2), hasItems(b1, b2)));
