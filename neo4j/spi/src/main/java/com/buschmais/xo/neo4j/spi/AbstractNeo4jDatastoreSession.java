...
public abstract class AbstractNeo4jDatastoreSession<N extends Neo4jNode, L extends Neo4jLabel, R extends Neo4jRelationship, T extends Neo4jRelationshipType>
    ...
    public void createIndexes(Set<Index> indexes) {
        ...
        for (Index index : indexes) {
            String statement = format("CREATE INDEX FOR (n:%s) ON (n.%s)", index.getLabels()
                .stream()
                .collect(joining(":")), index.getProperties()
                .stream()
                .collect(joining(",")));
    ...
    public String getNeo4jVersion() {
        try (ResultIterator<Map<String, Object>> iterator = createQuery(Cypher.class).execute(
            "CALL dbms.components() YIELD versions UNWIND versions AS version RETURN version", emptyMap())) {
            while (iterator.hasNext()) {
    ...
    private Set<Index> getIndexes(String labelsColumn, String propertiesColumn) {
        ...
        String query = format("SHOW INDEXES YIELD %s AS labels, %s AS properties WHERE labels is not null RETURN labels, properties", labelsColumn,
            propertiesColumn);
